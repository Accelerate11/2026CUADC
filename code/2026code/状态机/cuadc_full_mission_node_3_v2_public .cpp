...
namespace
{
constexpr const char * kMissionVersion = "cuadc-community-v2-2026-09";
...
class CuadcPublicMissionV2 final : public rclcpp::Node
{
public:
  CuadcPublicMissionV2()
  : Node("cuadc_public_mission_v2")
  {
    declare_parameters();
    load_parameters();
    ...
    state_sub_ = create_subscription<mavros_msgs::msg::State>(
      ...
      std::bind(&CuadcPublicMissionV2::state_callback, this, std::placeholders::_1));
    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>(
      "/mavros/local_position/odom", rclcpp::SensorDataQoS(),
      std::bind(&CuadcPublicMissionV2::odom_callback, this, std::placeholders::_1));
    compass_sub_ = create_subscription<std_msgs::msg::Float64>(
      "/mavros/global_position/compass_hdg", rclcpp::SensorDataQoS(),
      std::bind(&CuadcPublicMissionV2::compass_callback, this, std::placeholders::_1));
    bucket_sub_ = create_subscription<geometry_msgs::msg::PoseArray>(
      bucket_topic_, rclcpp::SensorDataQoS(),
      std::bind(&CuadcPublicMissionV2::bucket_callback, this, std::placeholders::_1));
    ...
    recon_ack_sub_ = create_subscription<std_msgs::msg::UInt32>(
      ...
      std::bind(&CuadcPublicMissionV2::recon_ack_callback, this, std::placeholders::_1));
    ...
    timer_ = create_wall_timer(50ms, std::bind(&CuadcPublicMissionV2::tick, this));
    RCLCPP_INFO(
      get_logger(),
      "CUADC V2 public mission node ready [%s]. Competition tuning/ranking omitted.",
      kMissionVersion);
  }
...
private:
  void declare_parameters()
  {
    ...
    // Conservative demo values; intentionally unrelated to competition tuning.
    ...
    declare_parameter<double>("transit_speed_m_s", 1.8);
    declare_parameter<double>("search_speed_m_s", 0.9);
    declare_parameter<double>("return_speed_m_s", 1.8);
    ...
    declare_parameter<double>("release_stabilization_s", 0.8);
    declare_parameter<double>("max_release_horizontal_speed_m_s", 0.25);
    declare_parameter<double>("max_release_vertical_speed_m_s", 0.20);
    ...
    // Demo target-set policy.  It illustrates preferred-count / fallback-count
    // architecture only; no competition scoring/ranking is included.
    declare_parameter<int>("demo_preferred_target_count", 3);
    declare_parameter<int>("demo_minimum_target_count", 2);
    declare_parameter<int>("demo_track_confirmations", 3);
    declare_parameter<double>("demo_track_gate_m", 0.8);
    declare_parameter<double>("demo_reacquire_position_gate_m", 1.0);
    declare_parameter<double>("demo_reacquire_size_gate_m", 0.10);
    ...
    declare_parameter<int>("demo_search_lanes", 4);
  ...
  void load_parameters()
  {
    ...
    release_stabilization_s_ = std::max(
      0.1, get_parameter("release_stabilization_s").as_double());
    max_release_horizontal_speed_m_s_ = std::max(
      0.05, get_parameter("max_release_horizontal_speed_m_s").as_double());
    max_release_vertical_speed_m_s_ = std::max(
      0.05, get_parameter("max_release_vertical_speed_m_s").as_double());
    ...
    preferred_target_count_ = std::max(
      2, static_cast<int>(get_parameter("demo_preferred_target_count").as_int()));
    minimum_target_count_ = std::max(
      1, static_cast<int>(get_parameter("demo_minimum_target_count").as_int()));
    minimum_target_count_ = std::min(minimum_target_count_, preferred_target_count_);
    track_confirmations_ = std::max(
      1, static_cast<int>(get_parameter("demo_track_confirmations").as_int()));
    track_gate_m_ = std::max(0.2, get_parameter("demo_track_gate_m").as_double());
    reacquire_position_gate_m_ = std::max(
      0.2, get_parameter("demo_reacquire_position_gate_m").as_double());
    reacquire_size_gate_m_ = std::max(
      0.01, get_parameter("demo_reacquire_size_gate_m").as_double());
  ...
  void bucket_callback(const geometry_msgs::msg::PoseArray::SharedPtr msg)
  {
    ...
    std::size_t temporary_id = 1U;
    for (const auto & pose : msg->poses) {
      Point3 body{pose.position.x, pose.position.y, pose.position.z};
      ...
      d.stamp = now();
      latest_detections_.push_back(d);
      update_demo_tracks(d);
    }
  }
  ...
  {
    // Public demo only.  Production capture-time interpolation and attitude
  ...
  {
    // Generic multi-lane search only.  Competition lane geometry/boundary guard
  ...
  // ---------------------------------------------------------------------------
  // Demo tracking / target-set logic
  // ---------------------------------------------------------------------------
  void update_demo_tracks(const Detection & d)
  {
    DemoTrack * nearest = nullptr;
    double nearest_distance = std::numeric_limits<double>::infinity();
    for (auto & track : tracks_) {
      const double distance = distance_xy(track.local, d.local);
      if (distance < nearest_distance && distance <= track_gate_m_) {
    ...
    // Simple exponential update for educational purposes only.
    nearest->local.x = 0.7 * nearest->local.x + 0.3 * d.local.x;
    nearest->local.y = 0.7 * nearest->local.y + 0.3 * d.local.y;
    nearest->local.z = 0.7 * nearest->local.z + 0.3 * d.local.z;
    nearest->size_hint = 0.7 * nearest->size_hint + 0.3 * d.size_hint;
    nearest->confidence = 0.7 * nearest->confidence + 0.3 * d.confidence;
    ++nearest->confirmations;
    nearest->last_seen = now();
  }

  std::vector<const DemoTrack *> ready_demo_tracks() const
  {
    std::vector<const DemoTrack *> ready;
    for (const auto & track : tracks_) {
      if (track.confirmations < track_confirmations_) {
        continue;
      }
      if ((now() - track.last_seen).seconds() > vision_timeout_s_ * 2.0) {
        continue;
      }
      bool used = false;
      for (const auto & p : released_target_points_) {
        if (distance_xy(track.local, p) < 0.6) {
          used = true;
          break;
        }
      }
  ...
  bool freeze_demo_target_plan(bool allow_minimum)
  {
    if (!selected_targets_.empty()) {
      return true;
    }

    auto ready = ready_demo_tracks();
    const std::size_t required = static_cast<std::size_t>(
      allow_minimum ? minimum_target_count_ : preferred_target_count_);
    if (ready.size() < required) {
      return false;
    }

    // Competition ranking is intentionally omitted.  Public version simply
    // chooses the nearest unused tracks to the aircraft.
    std::sort(
      ready.begin(), ready.end(), [this](const DemoTrack * a, const DemoTrack * b) {
        return distance_xy(a->local, position_) < distance_xy(b->local, position_);
      });

    selected_targets_.clear();
    const std::size_t count = std::min<std::size_t>(2U, ready.size());
    for (std::size_t i = 0U; i < count; ++i) {
      selected_targets_.push_back(*ready[i]);
    }
    return selected_targets_.size() >= 2U;
  }

  std::optional<DemoTrack> target_for_payload(std::size_t payload) const
  {
    if (payload >= selected_targets_.size()) {
      return std::nullopt;
    }
    return selected_targets_[payload];
  }

  bool try_demo_reacquire(DemoTrack & target)
  {
    // Production reacquisition uses stronger identity constraints.  Public
    // version only demonstrates position + size gating and never rewrites the
    // frozen mission reference.
    const Detection * best = nullptr;
    double best_distance = std::numeric_limits<double>::infinity();
    for (const auto & d : latest_detections_) {
      const double position_error = distance_xy(d.local, target.local);
      const double size_error = std::abs(d.size_hint - target.size_hint);
      if (position_error > reacquire_position_gate_m_ || size_error > reacquire_size_gate_m_) {
        continue;
      }
      bool used = false;
      for (const auto & p : released_target_points_) {
      ...
      if (!used && position_error < best_distance) {
    ...
    // Update only the active estimate; selected_targets_ remains frozen.
    target.local = best->local;
    target.confidence = best->confidence;
    return true;
  }

  Point3 desired_demo_align_pose(const DemoTrack & target) const
  {
    return Point3{target.local.x, target.local.y, home_->z + align_alt_m_};
  }

  Point3 desired_demo_release_pose(const DemoTrack & target) const
  {
    // Production release-port extrinsics intentionally omitted.
    return Point3{target.local.x, target.local.y, home_->z + release_alt_m_};
  }
    ...
    switch (state_) {
      ...
      case State::DONE:
        ...
        if (!done_logged_) {
          done_logged_ = true;
          RCLCPP_INFO(
            get_logger(), "V2 public mission ended: payloads=%zu recon_acks=%zu failed=%s",
  ...
  void update_search()
  {
    // Public architecture: prefer a larger stable target set while en route,
    // but allow the configured minimum at the end of the demo route.
    if (freeze_demo_target_plan(false)) {
      active_target_ = target_for_payload(payload_index_);
      if (active_target_.has_value()) {
        target_ = desired_demo_align_pose(*active_target_);
        enter(State::ALIGN);
        return;
      }
    }
    ...
    if (freeze_demo_target_plan(true)) {
      active_target_ = target_for_payload(payload_index_);
      if (active_target_.has_value()) {
        enter(State::ALIGN);
        return;
    ...
    fail_and_return("Public demo search finished without enough stable targets");
  }
  ...
  void update_align()
  {
    if (!active_target_.has_value()) {
      active_target_ = target_for_payload(payload_index_);
      if (!active_target_.has_value()) {
        fail_and_return("ALIGN entered without demo target");
        return;
      }
    }

    // Guarded demo reacquisition; exact competition logic intentionally omitted.
    (void)try_demo_reacquire(*active_target_);
    ...
    frozen_release_pose_ = desired_demo_release_pose(*active_target_);
  ...
  void update_release()
  {
    ...
    // Production V2 has stronger release gates. Public edition keeps only
    // generic position + velocity + dwell checks and safe output disable.
    target_ = frozen_release_pose_;
    const bool release_gate =
      distance_xy(position_, frozen_release_pose_) <= align_accept_radius_m_ &&
      std::abs(position_.z - frozen_release_pose_.z) <= 0.15 &&
      horizontal_speed_m_s_ <= max_release_horizontal_speed_m_s_ &&
      std::abs(vertical_speed_m_s_) <= max_release_vertical_speed_m_s_;
    ...
    if ((now() - *release_stable_since_).seconds() < release_stabilization_s_) {
      return;
    }
    ...
    released_target_points_.push_back(active_target_->local);
    ++payload_index_;
    active_target_.reset();
    ...
    active_target_ = target_for_payload(payload_index_);
    if (!active_target_.has_value()) {
      fail_and_return("Second frozen demo target unavailable");
      return;
    }
    enter(State::ALIGN);
  }
  ...
  {
    if (recon_ack_received(recon_index_) || (now() - state_enter_time_).seconds() > 1.0) {
      ++recon_index_;
  ...
  Point3 sample_segment() const
  {
    ...
    // Generic smoothstep only; production route/turn shaping is omitted.
  ...
  std::optional<DemoTrack> active_target_;
  Segment segment_;
  std::vector<Detection> latest_detections_;
  std::vector<DemoTrack> tracks_;
  std::vector<DemoTrack> selected_targets_;
  ...
  int preferred_target_count_ = 3;
  int minimum_target_count_ = 2;
  int track_confirmations_ = 3;
  int demo_search_lanes_ = 4;

  double track_gate_m_ = 0.8;
  double reacquire_position_gate_m_ = 1.0;
  double reacquire_size_gate_m_ = 0.10;
  ...
  double transit_speed_m_s_ = 1.8;
  double search_speed_m_s_ = 0.9;
  double return_speed_m_s_ = 1.8;
  double waypoint_accept_radius_m_ = 0.45;
  double align_accept_radius_m_ = 0.30;
  double align_stable_s_ = 0.8;
  double release_stabilization_s_ = 0.8;
  double max_release_horizontal_speed_m_s_ = 0.25;
  double max_release_vertical_speed_m_s_ = 0.20;
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<CuadcPublicMissionV2>());
  rclcpp::shutdown();
  return 0;
}
