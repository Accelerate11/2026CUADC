 ...
 *   -> TAKEOFF(4.0 m) -> SEARCH(2.0 m) -> ALIGN_COARSE(2.2 m)
 *   -> RELEASE(CH7) -> ALIGN_COARSE(2.2 m) -> RELEASE(CH8)
 *   -> RETURN_HOME(4.0 m) -> LAND -> DISARM -> DONE
 *
 * 设计原则：
 ...
 * 4. 2.0 m 搜索至少两个稳定、空间独立的桶；若同时看到更多桶，按直径降序优先冻结最大桶和次大桶。
 * 5. 两个投放口使用独立机体 FLU 外参：
 *      CH7: [ 0.026, -0.065, -0.32 ]
 *      CH8: [-0.026,  0.065, -0.32 ]
 * 6. 不再做厘米级精对准。目标锁定后在 2.2 m 做单阶段宽容对准，
 ...
 * 9. 返航高度固定为 takeoff_alt_m；落地采用相对高度和速度连续稳定确认后显式上锁。
 *
 * 位置源：/mavros/local_position/odom（ArduPilot EKF 融合 RTK/IMU/气压计后的本地位置）
 * 视觉源：/perception/drop_buckets_body
 * 舵机：MAV_CMD_DO_SET_SERVO，CH7/CH8，1100 us 挂载，1350 us 释放。
 */
...
constexpr const char * kMissionVersion = "drop-only-rescue-v2-2026-08-08-r1";
...
public:
  ...
  : Node("visual_drop_mission_node")
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "Visual drop node ready [%s]: manual GUIDED + %s arm, "
      "2.0m search / 2.2m rescue-priority two-bottle drop",
      kMissionVersion, auto_arm_on_guided_ ? "automatic" : "manual");
  }
...
private:
  void declare_parameters()
  {
    ...
    declare_parameter<bool>("rescue_reliability_profile", true);
    declare_parameter<bool>("enforce_final_drop_heights", true);
    declare_parameter<double>("search_alt_m", 2.0);
    declare_parameter<double>("coarse_alt_m", 2.2);
    declare_parameter<double>("fine_alt_m", 2.2);
    ...
    declare_parameter<int>("max_search_passes", 4);
    ...
    declare_parameter<double>("bucket_target_ranking_stable_s", 0.50);
    ...
    declare_parameter<double>("coarse_error_m", 0.35);
    declare_parameter<double>("fine_error_m", 0.08);
    declare_parameter<double>("coarse_stable_s", 0.5);
    declare_parameter<double>("fine_stable_s", 0.8);
    declare_parameter<double>("alignment_timeout_s", 12.0);
    declare_parameter<int>("max_drop_alignment_retries", 1);
    ...
    // 1.8 m 冻结目标直接投放的独立门限；YAML 未配置时采用这些比赛实测默认值。
    declare_parameter<double>("direct_release_xy_tolerance_m", 0.35);
    declare_parameter<double>("direct_release_height_tolerance_m", 0.15);
    declare_parameter<double>("direct_release_stable_s", 0.50);
    declare_parameter<double>("direct_release_timeout_s", 8.0);
    declare_parameter<double>("direct_release_max_horizontal_speed_m_s", 0.15);
    declare_parameter<double>("direct_release_max_vertical_speed_m_s", 0.10);
    declare_parameter<double>("direct_release_max_tilt_deg", 8.0);
    declare_parameter<double>("direct_release_max_angular_rate_deg_s", 10.0);
    ...
    declare_parameter<std::vector<int64_t>>(
      "servo_release_pwm", std::vector<int64_t>{1350, 1350});
    declare_parameter<std::vector<double>>(
  ...
  void load_parameters()
  {
    ...
    rescue_reliability_profile_ =
      get_parameter("rescue_reliability_profile").as_bool();
    const bool enforce_final_drop_heights =
      get_parameter("enforce_final_drop_heights").as_bool();
    if (enforce_final_drop_heights) {
      // Rescue V2：保留2.0 m低空搜索；投放升到2.2 m，降低机械/桶碰撞风险。
      search_alt_m_ = 2.0;
      coarse_alt_m_ = 2.2;
      fine_alt_m_ = 2.2;  // 仅保留参数兼容，状态机不再进入精对准。
    } else {
    ...
    required_bucket_count_ = rescue_reliability_profile_ ?
      2 : std::max(2, static_cast<int>(
      get_parameter("bucket_required_count").as_int()));
    ...
    max_drop_alignment_retries_ = std::max(
      0, static_cast<int>(get_parameter("max_drop_alignment_retries").as_int()));
    ...
    direct_release_stable_s_ = std::max(
      0.2, get_parameter("direct_release_stable_s").as_double());
    direct_release_timeout_s_ = std::max(
      2.0, get_parameter("direct_release_timeout_s").as_double());
    direct_release_max_horizontal_speed_m_s_ = std::max(
      0.03, get_parameter("direct_release_max_horizontal_speed_m_s").as_double());
    direct_release_max_vertical_speed_m_s_ = std::max(
      0.02, get_parameter("direct_release_max_vertical_speed_m_s").as_double());
    direct_release_max_tilt_rad_ = std::max(
      1.0, get_parameter("direct_release_max_tilt_deg").as_double()) *
      kPi / 180.0;
    direct_release_max_angular_rate_rad_s_ = std::max(
      1.0, get_parameter("direct_release_max_angular_rate_deg_s").as_double()) *
      kPi / 180.0;

    // Rescue V2可靠性优先：即使沿用旧YAML，也强制应用比赛现场的宽容投放门限。
    // B区为桶心周围直径1 m的有效区域，因此这里不再追求10 cm级精对准。
    if (rescue_reliability_profile_) {
      search_alt_m_ = 2.0;
      coarse_alt_m_ = 2.2;
      fine_alt_m_ = 2.2;
      required_bucket_count_ = 2;
      max_position_deviation_m_ = 0.35;
      coarse_error_m_ = 0.35;
      coarse_stable_s_ = 0.50;
      direct_release_xy_tolerance_m_ = 0.35;
      direct_release_height_tolerance_m_ = 0.15;
      direct_release_stable_s_ = 0.50;
      direct_release_timeout_s_ = 8.0;
      direct_release_max_horizontal_speed_m_s_ = 0.15;
      direct_release_max_vertical_speed_m_s_ = 0.10;
      direct_release_max_tilt_rad_ = 8.0 * kPi / 180.0;
      direct_release_max_angular_rate_rad_s_ = 10.0 * kPi / 180.0;
      max_drop_alignment_retries_ = 1;
    }
    ...
    config_valid_ =
      ...
      release_pwm_[0] == 1350 && release_pwm_[1] == 1350 &&
    ...
    if (!config_valid_) {
      RCLCPP_ERROR(
        ...
        "servo mode, two payloads, CH7/CH8=1100->1350 for 0.7s, and safe stow");
    }
  }
  ...
  void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
  {
    ...
    // Camera capture stamps are sampled from the NUC ROS clock immediately
    // after wait_for_frames(). Stamp odom history with the same receipt clock
    // so interpolation cannot be broken by FCU/MAVLink header-clock skew.
  ...
  {
    // SEARCH 必须视觉；ALIGN_COARSE 可用新帧修正轨迹。进入 RELEASE 后冻结位姿，
    // 不再让视场变化、ID 重建或目标暂时出画影响投放。
  ...
  bool try_lock_target_plan()
  {
    ...
    // 先按轨迹质量排序，去掉空间上重复/过近的轨迹；不再要求必须看齐三个桶。
    std::sort(
    ...
    const std::size_t needed = static_cast<std::size_t>(
      std::max(payload_count_, required_bucket_count_));
    if (distinct.size() < needed) {
      ranking_candidate_ids_.clear();
      target_ranking_stable_since_.reset();
      RCLCPP_INFO_THROTTLE(
        get_logger(), *get_clock(), 1500,
        "Waiting for stable distinct rescue buckets: ready=%zu required=%zu known=%zu",
        distinct.size(), needed, known_buckets_.size());
      return false;
    }

    // 救援可靠性优先：在当前所有稳定、空间独立的候选中，优先选择直径最大的两个。
    // 若只稳定识别到两个桶，则直接使用这两个，不再为了等第三个桶而浪费任务时间。
    std::sort(
      distinct.begin(), distinct.end(),
      [](const BucketTrack * a, const BucketTrack * b) {
    ...
    for (int index = 0; index < payload_count_; ++index) {
      proposed_ids.push_back(distinct[static_cast<std::size_t>(index)]->id);
    }
    ...
    if (proposed_ids != ranking_candidate_ids_) {
      ...
      RCLCPP_INFO(
        get_logger(),
        "Rescue target candidate: payload1=#%zu diameter=%.3fm, "
        "payload2=#%zu diameter=%.3fm; holding %.1fs before freeze",
    ...
    selected_target_ids_.assign(
      ranking_candidate_ids_.begin(), ranking_candidate_ids_.end());
    selected_target_positions_.clear();
    selected_target_diameters_.clear();
    for (int index = 0; index < payload_count_; ++index) {
      const BucketTrack * selected = distinct[static_cast<std::size_t>(index)];
      selected_target_positions_.push_back(selected->local);
      selected_target_diameters_.push_back(selected->diameter);
    }
    target_selection_locked_ = true;
    RCLCPP_INFO(
      get_logger(),
      "RESCUE_TARGETS_LOCKED: payload1 largest=#%zu diameter=%.3fm; "
      "payload2 next=#%zu diameter=%.3fm",
      selected_target_ids_[0], selected_target_diameters_[0],
      selected_target_ids_[1], selected_target_diameters_[1]);
    return true;
  }
  ...
  {
    // 只有搜索阶段必须持续有时间对齐的视觉。三桶计划冻结后，后续对准和投放
    // 可以依靠冻结的 local 坐标继续执行，避免低高度视场变化导致误返航。
  ...
  void begin_visual_failsafe(const std::string & reason)
  {
    ...
    const bool can_return = state_ != State::TAKEOFF && home_.has_value() &&
      guided_active_ && odom_fresh() && compass_fresh() &&
  ...
  void tick()
  {
    ...
    if ((now() - last_status_time_).seconds() >= 4.0) {
      ...
      RCLCPP_INFO(
        ...
        std::min(payload_index_ + 1U, static_cast<std::size_t>(payload_count_)),
        known_buckets_.size(),
  ...
  void start_search()
  {
    ...
    enter(State::SEARCH);
    RCLCPP_INFO(
      get_logger(), "Start rescue drop-area search pass %d/%d for payload %zu",
  ...
  void update_search()
  {
    ...
    if (search_pass_ < max_search_passes_) {
      ...
      RCLCPP_ERROR(
        get_logger(),
        "Fewer than two stable rescue buckets after %d passes; preserving remaining payload and returning",
        max_search_passes_);
  ...
  std::optional<BucketTrack> frozen_target_for_payload(std::size_t payload) const
  {
    if (!target_selection_locked_ || payload >= selected_target_ids_.size() ||
    ...
    BucketTrack frozen;
    frozen.id = selected_target_ids_[payload];
    frozen.local = selected_target_positions_[payload];
    frozen.diameter = selected_target_diameters_[payload];
    frozen.confidence = 1.0;
    frozen.position_deviation = 0.0;
    frozen.diameter_deviation = 0.0;
    frozen.confirmations = static_cast<std::size_t>(min_confirmations_);
    frozen.stamp = now();
    return frozen;
  }
  ...
  bool retry_alignment_or_return(const std::string & stage)
  {
    if (drop_alignment_retry_count_ < max_drop_alignment_retries_) {
      ++drop_alignment_retry_count_;
      direct_release_active_ = false;
      forced_release_ = false;
      forced_release_stable_since_.reset();
      alignment_stable_since_.reset();
      ...
      RCLCPP_WARN(
        get_logger(),
        "RESCUE_DROP_RETRY: payload=%zu stage=%s retry=%d/%d; "
        "re-aligning same frozen target instead of returning",
        payload_index_ + 1U, stage.c_str(), drop_alignment_retry_count_,
        max_drop_alignment_retries_);
      enter(State::ALIGN_COARSE);
      return true;
    }

    const std::string reason =
      "Rescue drop alignment failed after retry; payload preserved";
    mark_mission_failure(reason);
    RCLCPP_ERROR(
      get_logger(),
      "RESCUE_DROP_GIVEUP: payload=%zu stage=%s; preserving payload and returning",
      payload_index_ + 1U, stage.c_str());
    direct_release_active_ = false;
    start_return_home();
    return false;
  }
  ...
  void update_alignment(bool /* fine */)
  {
    if (!active_bucket_.has_value() || !alignment_start_time_.has_value()) {
      mark_mission_failure("Alignment entered without a frozen target");
      start_return_home();
      return;
    }

    if ((now() - *alignment_start_time_).seconds() >= alignment_timeout_s_) {
      retry_alignment_or_return("ALIGN_COARSE_TIMEOUT");
      return;
    }

    // 单阶段救援对准：把对应投放口移动到目标桶心上方2.2 m。
    // 不追求厘米级A区命中；0.35 m门限优先确保水瓶能够及时投出并争取B区有效分。
    target_ = desired_release_pose(coarse_alt_m_);
    const double position_error = distance_xy(position_, target_);
    const double compensated_error = release_error();
    const bool aligned =
      position_error <= coarse_error_m_ &&
      compensated_error <= coarse_error_m_ &&
    ...
    if ((now() - *alignment_stable_since_).seconds() < coarse_stable_s_) {
      return;
    }

    // 对准完成后冻结最终补偿位姿。RELEASE 不再读取后续视觉。
    direct_release_active_ = true;
    forced_release_ = false;
    direct_release_pose_ = desired_release_pose(coarse_alt_m_);
    ...
    RCLCPP_WARN(
      get_logger(),
      "DIRECT_RELEASE_AFTER_ALIGNMENT: payload=%zu target_id=%zu error=%.3f "
      "aircraft_alt=%.2f outlet_alt≈%.2f; target pose frozen",
      payload_index_ + 1U, active_bucket_->id, compensated_error,
      coarse_alt_m_, coarse_alt_m_ +
      vector3_at(release_offsets_, payload_index_, Point3{}).z);
    enter(State::RELEASE);
  }
  ...
  void update_release()
  {
    ...
    if (!release_started_ && !release_command_pending_) {
      ...
      const bool area_ok =
        ...
        inside_release_area(direct_release_pose_);
      const double xy_error = distance_xy(position_, direct_release_pose_);
      ...
      if (!gate_ok) {
        ...
        RCLCPP_INFO_THROTTLE(
          ...
          "Direct release waiting: identity=%s area=%s xy=%.3f/%.3f "
          "z=%.3f/%.3f hspeed=%.3f/%.3f vz=%.3f/%.3f",
        ...
        if ((now() - state_enter_time_).seconds() >= direct_release_timeout_s_) {
  ...
  {
    RCLCPP_INFO(
      get_logger(), "Payload %zu complete on target id=%zu diameter=%.3f",
    ...
    // 第二瓶直接使用首次冻结的第二救援目标；不重新搜索、不依赖新ID。
    const auto frozen = frozen_target_for_payload(payload_index_);
    if (!frozen.has_value() || target_id_already_used(frozen->id)) {
      mark_mission_failure("Frozen second target unavailable after first release");
      start_return_home();
      return;
    }
    active_bucket_ = *frozen;
    drop_alignment_retry_count_ = 0;
    alignment_start_time_ = now();
    alignment_stable_since_.reset();
    target_ = desired_release_pose(coarse_alt_m_);
    RCLCPP_INFO(
      get_logger(),
      "Using frozen second rescue target for payload %zu: id=%zu diameter=%.3f "
      "local=(%.2f,%.2f)",
      payload_index_ + 1U, active_bucket_->id, active_bucket_->diameter,
  ...
  void update_recover()
  {
    const auto frozen = frozen_target_for_payload(payload_index_);
    if (!frozen.has_value()) {
      mark_mission_failure("RECOVER could not load frozen target");
      start_return_home();
      return;
    }
    active_bucket_ = *frozen;
  ...
  bool direct_release_active_ = false;
  bool rescue_reliability_profile_ = true;
  ...
  double coarse_alt_m_ = 2.2;
  double fine_alt_m_ = 2.2;
  ...
  double target_ranking_stable_s_ = 0.50;
  ...
  double coarse_error_m_ = 0.35;
  double fine_error_m_ = 0.08;
  double coarse_stable_s_ = 0.5;
  ...
  double direct_release_xy_tolerance_m_ = 0.35;
  double direct_release_height_tolerance_m_ = 0.15;
  double direct_release_stable_s_ = 0.50;
  double direct_release_timeout_s_ = 8.0;
  double direct_release_max_horizontal_speed_m_s_ = 0.15;
  double direct_release_max_vertical_speed_m_s_ = 0.10;
  double direct_release_max_tilt_rad_ = 8.0 * kPi / 180.0;
  double direct_release_max_angular_rate_rad_s_ = 10.0 * kPi / 180.0;
  ...
  int max_drop_alignment_retries_ = 1;
  int drop_alignment_retry_count_ = 0;
  int search_pass_ = 0;
